 ...
 */
public final class HeliportValidator implements Predicate<ArincRecord> {
  private static final Logger LOG = LoggerFactory.getLogger(HeliportValidator.class);

  private final BiConsumer<ArincRecord, String> missingFieldConsumer;

  public HeliportValidator() {
    this((arincRecord, field) -> LOG.debug("Missing required field {} in record {}.", field, arincRecord.rawRecord()));
  }
  public HeliportValidator(BiConsumer<ArincRecord, String> missingFieldConsumer) {
  ...
  public boolean test(ArincRecord arincRecord) {
    return isCorrectSectionSubSection(arincRecord)
        ...
        && containsParsedField(arincRecord, "heliportIdentifier", missingFieldConsumer)
        && containsParsedField(arincRecord, "heliportIcaoRegion", missingFieldConsumer);
  }
  ...
  boolean isCorrectSectionSubSection(ArincRecord arincRecord) {
    ...
    return sectionCode.filter(SectionCode.H::equals).isPresent() && subSectionCode.filter("A"::equals).isPresent();
  }
}

...
class TestBoogieProcedure {

  @Test
  void testEqualsAndHashCode() {
    EqualsVerifier.forClass(BoogieProcedure.class).verify();
  }
  ...
  void testToFromBuilder() {
    ...
    BoogieProcedure procedure = new BoogieProcedure.Builder()
        .procedureIdentifier("TEST")
        .airportIdentifier("TEST")
        .airportRegion("TEST")
        .procedureType(ProcedureType.STAR)
        .requiredNavigationEquipage(RequiredNavigationEquipage.RNP)
        .transitions(singletonList(transition))
        .build();

    assertEquals(procedure, procedure.toBuilder().build(), "toBuilder().build() should return equal procedure.");
  }
}

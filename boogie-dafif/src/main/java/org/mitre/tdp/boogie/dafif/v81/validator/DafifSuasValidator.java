...
public final class DafifSuasValidator implements Predicate<DafifRecord> {

  private static final Logger LOG = LoggerFactory.getLogger(DafifSuasValidator.class);

  private final BiConsumer<DafifRecord, String> missingFieldConsumer;

  public DafifSuasValidator() {
    this((record, field) -> LOG.debug("Missing required field {} in record {}.", field, record.rawRecord()));
  }

  public DafifSuasValidator(BiConsumer<DafifRecord, String> missingFieldConsumer) {
    this.missingFieldConsumer = requireNonNull(missingFieldConsumer);
  }

  @Override
  public boolean test(DafifRecord record) {
    return record.recordType().equals(DafifRecordType.SUAS)

...
public final class DafifBoundaryValidator implements Predicate<DafifRecord> {

  private static final Logger LOG = LoggerFactory.getLogger(DafifBoundaryValidator.class);

  private final BiConsumer<DafifRecord, String> missingFieldConsumer;

  public DafifBoundaryValidator() {
    this((record, field) -> LOG.debug("Missing required field {} in record {}.", field, record.rawRecord()));
  }

  public DafifBoundaryValidator(BiConsumer<DafifRecord, String> missingFieldConsumer) {
    this.missingFieldConsumer = requireNonNull(missingFieldConsumer);
  }

  @Override
  public boolean test(DafifRecord record) {
    return record.recordType().equals(DafifRecordType.BDRY)
        && containsParsedField(record, "boundaryIdentification", missingFieldConsumer)
        && containsParsedField(record, "segmentNumber", missingFieldConsumer)
        && containsParsedField(record, "boundaryType", missingFieldConsumer)
        && containsParsedField(record, "icaoCode", missingFieldConsumer)
        && containsParsedField(record, "shape", missingFieldConsumer)
        && (!("L".equals(record.requiredField("shape")) || "R".equals(record.requiredField("shape")))
            || containsParsedField(record, "derivation", missingFieldConsumer))

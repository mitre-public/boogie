...
public final class CiDelegator implements BiPredicate<ConformablePoint, FlyableLeg> {
  ...
  public boolean test(ConformablePoint conformablePoint, FlyableLeg flyableLeg) {
    return PathTerminator.CI.equals(flyableLeg.current().pathTerminator())
        && validator.test(flyableLeg.current())
        && conformablePoint.trueCourse().isPresent()
        && flyableLeg.next().map(Leg::associatedFix).isPresent();
  }
}

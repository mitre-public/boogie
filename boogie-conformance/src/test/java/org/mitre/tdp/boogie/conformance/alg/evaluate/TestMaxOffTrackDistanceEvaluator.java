...
class TestMaxOffTrackDistanceEvaluator {
  @Test
  void LegDistance_OnLeg_IsRadialDistance() {
    ...
    Distance dist = MaxOffTrackDistanceEvaluator.offTrackDistance(pt, mockLegPair()).orElseThrow(() -> new IllegalStateException());
    assertEquals(60., dist.inNauticalMiles(), 0.01);
  }

  @Test
  void LegDistance_OffLeg_IsEndpointDistance() {
    ConformablePoint pt = mock(ConformablePoint.class, withSettings().defaultAnswer(CALLS_REAL_METHODS));
  ...
  static LegPair mockLegPair() {
    ...
    LegPair legPair = new LegPairImpl(previousLeg, currentLeg);
